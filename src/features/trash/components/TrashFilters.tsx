...
import TrashFilterControls from "./TrashFilterControls";

type TrashFiltersProps = {
  filterControls: UseTrashFiltersReturn;
};

const TrashFilters: React.FC<TrashFiltersProps> = ({ filterControls }) => {
  ...
  return (
    <div className="space-y-4">
          ...
          <Drawer open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DrawerContent className="p-6 border-muted">
              <DrawerHeader className="p-0 mb-0">
                <DrawerTitle className="text-left">Sort Trash Items</DrawerTitle>
              </DrawerHeader>
              <DrawerDescription>Choose how to sort deleted items.</DrawerDescription>

              <div className="mt-6 space-y-4">
                <TrashFilterControls filterControls={filterControls} isMobile onMobileClose={handleMobileClose} />
              </div>
            </DrawerContent>
          </Drawer>
        </div>
      ...
      <div className="hidden md:flex md:items-end md:justify-between md:gap-4">
        <TrashFilterControls filterControls={filterControls} />
      </div>
    </div>
  );
};
