// src/features/folder/api.ts
import api from "@/lib/api";
import { z } from "zod";
import { FolderSchema, FolderListResponseSchema, CreateFolderRequestSchema, MoveFolderRequestSchema } from "./schemas";
...
export const createFolder = async (data: CreateFolderRequest): Promise<Folder> => {
  const response = await api.post<ApiResponse<(typeof FolderSchema)["_type"]>>("/folders", data);
  return FolderSchema.parse(response.data.data);
};

export const getRootFolders = async (crateId: string): Promise<Folder[]> => {
  const response = await api.get<ApiResponse<(typeof FolderListResponseSchema)["_type"]>>(
    `/crates/${crateId}/folders/root`
  );
