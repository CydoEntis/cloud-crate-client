...
function GetMoreStorage({ user }: GetMoreStorageProps) {
  const percent = user.accountStorageLimitBytes > 0 ? (user.allocatedStorageBytes / user!.allocatedStorageBytes) * 100 : 0;
  ...
  return (
    <div className="space-y-2 w-full">
      ...
      <p className="text-sm text-muted-foreground text-center flex justify-between items-center">
        {formatBytes(user!.remainingUsageBytes)} of {formatBytes(user!.remainingAllocationBytes)} used
      </p>
